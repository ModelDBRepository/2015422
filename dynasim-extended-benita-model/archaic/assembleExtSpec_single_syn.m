function specification = assembleExtSpec_single_syn(numCellsScale)
...
% AES yes need to remove the second spikes argument for 2021b syns that
% need spiking
...
eqns={
  ...
  'Cm = 1'    % uF/cm^2
  'spike_threshold = -25'
  'monitor v.spikes(spike_threshold, 2)'
  'vIC = -68'    % mV
  'vNoiseIC = 50' % mV
...
specification.connections(3).direction='TC<-TRN';
specification.connections(3).mechanism_list={...
    'iGABAA_TC_TRN_AS17',...
    'iGABAB_TC_TRN_AS17',...
    };
specification.connections(4).direction='TRN<-TRN';
specification.connections(4).mechanism_list={'iGABAA_TRN_TRN_AS17'};
specification.connections(5).direction='TRN<-TC';
specification.connections(5).mechanism_list={'iAMPA_TRN_TC_AS17'};

% -------------------------------------------------------------------
%% 4. Thalamo-cortical Connections
% -------------------------------------------------------------------
specification.connections(6).direction='PYdr<-TC';
specification.connections(6).mechanism_list={'iAMPA_PYdr_TC'};

specification.connections(7).direction='IN<-TC';
specification.connections(7).mechanism_list={'iAMPA_IN_TC'};

specification.connections(8).direction='TC<-PYso';
specification.connections(8).mechanism_list={'iAMPA_TC_PYso'};

specification.connections(9).direction='TRN<-PYso';
specification.connections(9).mechanism_list={'iAMPA_TRN_PYso'};
